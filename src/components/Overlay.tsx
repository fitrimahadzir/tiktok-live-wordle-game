...
import { Trophy, Flame, Users, Gift, Heart, UserPlus, SkipForward } from "lucide-react";
...
export default function Overlay({ gameState, notifications, tiktokStatus, adminAction }: any) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-dark-bg dark:bg-slate-900 transition-colors duration-500">
      <div className="w-[1280px] h-[768px] relative mx-auto bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-sans overflow-hidden flex flex-col p-6 border border-slate-200 dark:border-slate-700 select-none transition-colors duration-500 shadow-2xl dark:shadow-[0_0_50px_rgba(0,0,0,0.5)]">
        ...
        {/* Main Gameplay Area: Centering the group (Leaderboard + Board) as one unit */}
        <div className="flex-1 flex gap-8 items-center justify-center min-h-0">
          ...
          <div className="w-64 h-fit flex flex-col gap-4">
            ...
            {/* HypeMeter hidden by request */}
          </div>

          {/* Center Column: Game Board (Part of the centered group) */}
          <div className="w-fit flex flex-col items-center justify-center">
            <TekataBoard guesses={gameState.guesses} wordLength={gameState.wordLength || 5} category={gameState.currentCategory} />

            {/* Like Bar + Heart Reminder */}
            <div className="mt-6 w-full max-w-md space-y-2">
              ...
              <div className="flex items-center gap-3 bg-gradient-to-r from-pink-500/10 to-red-500/10 border border-pink-500/30 rounded-2xl px-4 py-3 relative overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-[10px] font-black uppercase tracking-[0.2em] text-pink-400 mb-0.5">💖 SUPPORT HOST</div>
                  <div className="text-sm font-bold text-white leading-tight">Jangan lupa send <span className="text-pink-400">Heart Me</span> untuk tekaan unlimited! 🎁</div>
                </div>
              ...
              <div className="bg-white/5 dark:bg-slate-900/50 border border-white/10 dark:border-slate-700 rounded-2xl px-4 py-3 transition-colors duration-500">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 dark:text-slate-500">Jumlah Like</span>
                  </div>
                  <span className="text-xs font-black text-neon-cyan font-mono">
                    {(gameState.hypeInfo?.current || 0).toLocaleString()} / {(gameState.hypeInfo?.target || 1000).toLocaleString()}
                  </span>
                </div>
                <div className="h-2 w-full bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                  <motion.div
                    animate={{ width: `${Math.min(100, ((gameState.hypeInfo?.current || 0) / (gameState.hypeInfo?.target || 1000)) * 100)}%` }}
              ...
              <div className="text-[8px] text-slate-500 dark:text-slate-600 text-center mt-3 uppercase tracking-wider font-bold">
                Develop By <a href="https://fitrimahadzir.my" target="_blank" rel="noopener noreferrer" className="text-neon-cyan hover:underline">Fitri Mahadzir</a>
              </div>
            </div>
          </div>
        </div>
            ...
            >
              ...
              >
                ...
                <div className="space-y-1 mb-8">
                  <h3 className="text-neon-yellow font-black text-xl uppercase tracking-[0.2em]">KITA ADA PEMENANG!</h3>
                  <h2 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white">@{gameState.winner.nickname}</h2>
                </div>

                <div className="w-full h-px bg-slate-100 dark:bg-slate-700 mb-8" />

                <div className="space-y-2 mb-10">
                  <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Berjaya teka perkataan:</p>
          ...
          <AnimatePresence>
            {notifications.map((n: any, i: number) => (
              <motion.div
                ...
                className={`bg-white dark:bg-slate-800 border-2 ${n.type === 'powerup' ? 'border-neon-yellow' : 'border-neon-purple'} p-3 px-6 rounded-2xl flex items-center gap-4 shadow-xl ring-4 ring-white dark:ring-slate-900 transition-colors duration-500`}
              >
                {n.type === "powerup" ? (
                  <>
                  ...
                  <>
                    ...
                    <div>
                      <div className="text-[10px] text-[#0891B2] dark:text-[#22D3EE] font-bold uppercase">{n.type === "gift" ? "Hadiah TikTok" : "Pengikut Baru"}</div>
