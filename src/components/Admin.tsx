...
import { Play, SkipForward, RotateCcw, Monitor, Gamepad2, Link2, AlertCircle, Apple, Palette, Map, Globe, Tv, X, Eye, Moon, ChevronDown, Code } from "lucide-react";
...
export default function Admin({ isOpen, onClose, connectTiktok, tiktokStatus, adminAction, gameState, showAnswer, setShowAnswer, isDarkMode, setIsDarkMode, isDevMode, setIsDevMode }: AdminProps) {
      ...
      >
        ...
        <div className="flex-1 p-5 flex flex-col gap-6">
            ...
            <div className="flex items-center gap-3 text-sm bg-slate-900/50 p-3 rounded-xl border border-slate-800">
              <div className="flex items-center gap-2">
                <div className={clsx(
                  ...
                  isConnected ? "bg-neon-green shadow-neon-green animate-pulse" : "bg-red-500 shadow-[0_0_8px_#EF4444]"
                )} />
                <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">
                  {isConnected ? "Bersambung" : tiktokStatus?.status === "connecting" ? "Menyambung..." : "Tidak Disambung"}
                </span>
              </div>
              {gameState?.tiktokUsername && (
                <span className="text-neon-cyan font-mono text-xs ml-auto bg-neon-cyan/10 px-2 py-0.5 rounded border border-neon-cyan/20">@{gameState.tiktokUsername}</span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between p-3 bg-slate-900 border border-slate-700 rounded-xl">
                <div className="flex items-center gap-3">
                  <div className={clsx(
                    ...
                    showAnswer ? "bg-neon-green/10 text-neon-green" : "bg-slate-800 text-slate-500"
                  )}>
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    showAnswer ? "bg-neon-green shadow-[0_0_8px_#4ADE80]" : "bg-slate-700"
                  )}
                >
                  <div className={clsx(
              ...
              <div className="flex items-center justify-between p-3 bg-slate-900 border border-slate-700 rounded-xl">
                <div className="flex items-center gap-3">
                  <div className={clsx(
                    "p-2 rounded-lg transition-colors",
                    isDarkMode ? "bg-neon-purple/10 text-neon-purple" : "bg-slate-800 text-slate-500"
                  )}>
                ...
                <button
                  ...
                  className={clsx(
                    "w-12 h-6 rounded-full transition-all relative",
                    isDarkMode ? "bg-neon-purple shadow-[0_0_8px_#8B5CF6]" : "bg-slate-700"
                  )}
                >
                  <div className={clsx(
                    "absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-sm",
              ...
              <div className="flex items-center justify-between p-3 bg-slate-900 border border-slate-700 rounded-xl">
                <div className="flex items-center gap-3">
                  <div className={clsx(
                    "p-2 rounded-lg transition-colors",
                    isDevMode ? "bg-neon-cyan/10 text-neon-cyan" : "bg-slate-800 text-slate-500"
                  )}>
                ...
                <button
                  ...
                  className={clsx(
                    "w-12 h-6 rounded-full transition-all relative",
                    isDevMode ? "bg-neon-cyan shadow-[0_0_8px_#0891B2]" : "bg-slate-700"
                  )}
                >
                  <div className={clsx(
                    "absolute top-1 w-4 h-4 rounded-full bg-white transition-all shadow-sm",
          ...
          <div className="bg-slate-800/50 shadow-sm p-4 rounded-2xl border border-slate-700/50 space-y-4">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-neon-cyan">Skip Perkataan</span>
              </button>
              <button
              ...
              >
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400 group-hover:text-red-500">Reset Permainan</span>
              </button>
            </div>
          </div>

          {/* === BAHAGIAN 3: Pilihan Kategori === */}
                ...
                {categories.map((cat) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setIsCategoryDropdownOpen(false); // Optionally close after select, but better to let them select again if they want, let's keep it open
                    }}
                    className={clsx(
          ...
          <div className="p-4 bg-neon-purple/5 border border-neon-purple/20 rounded-2xl mt-auto">
            <h2 className="text-[10px] font-bold text-neon-purple uppercase tracking-widest mb-3">Arahan Persediaan OBS</h2>
            <ol className="list-decimal list-outside ml-3 space-y-1.5 text-[11px] text-slate-400 leading-relaxed">
              <li>Masukkan nama pengguna TikTok dan klik <span className="text-white">Sambung</span>.</li>
              <li>Buka OBS atau TikTok LIVE Studio.</li>
              <li>Tambah <span className="text-neon-cyan">Sumber Pelayar (Browser Source)</span>.</li>
              <li>Tetapkan URL ke halaman ini.</li>
              <li>Tetapkan Resolusi: <span className="text-white">1280×768</span>.</li>
